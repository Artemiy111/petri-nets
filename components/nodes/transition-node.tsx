...
import type { TransitionData } from "@/lib/petri-net"

type TransitionNodeProps = NodeProps<TransitionData> & {
  showLabels: boolean
  showNumbers: boolean
}
  ...
  return (
    <div className="relative">
      <div
        ...
        } ${getBackgroundColor()} ${canFire && !isWaiting ? "cursor-pointer" : "cursor-not-allowed"}`}
        style={{ transition: "background-color 0.15s ease" }}
        onClick={handleClick}
      >
        {/* Display delay if greater than 0 */}
        {delay > 0 && (
          <div className="absolute top-1 right-1 text-xs font-semibold bg-white/80 px-1 rounded">τ={delay}</div>
        )}

        {/* Индикатор состояния "метки забраны" */}
        {tokensRemoved && (
          <div className="absolute bottom-1 left-1 text-xs font-semibold bg-white/80 px-1 rounded">⏳</div>
        )}
      </div>
